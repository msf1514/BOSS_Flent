...
const data = JSON.parse(
  readFileSync(resolve(root, 'data/prototype.json'), 'utf8'),
);
...
assert.equal(
  data.marketEvidence.decision_boundary.includes('does not recommend'),
  true,
);
assert.equal(data.case.referenceDecision.generatedByPrototype, false);
assert.equal(
  data.economics.scenarios.find((s) => s.id === 'opening_ask')
    .simplePaybackMonths,
  35.1,
);
assert.equal(
  data.economics.scenarios.find((s) => s.id === 'verbal_54000')
    .simplePaybackMonths,
  27.7,
);
assert.equal(
  data.economics.scenarios.find((s) => s.id === 'room_2_downside_fixed_deposit')
    .simplePaybackMonths,
  57.1,
);
assert.equal(
  data.economics.scenarios.find(
    (s) => s.id === 'room_2_downside_linked_deposit',
  ).simplePaybackMonths,
  58.8,
);
assert.equal(
  data.listings.filter((r) => r.decision.b2State === 'include').length,
  8,
);
assert.equal(
  data.listings.filter((r) => r.decision.b2State === 'duplicate_collapsed')
    .length,
  2,
);
assert.equal(
  data.listings.filter((r) => r.decision.b2State === 'needs_human_review')
    .length,
  2,
);
